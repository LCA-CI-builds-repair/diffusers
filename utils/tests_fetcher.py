...
# you may not use this file except in compliance     tests = [f"tests/{f}" for f in tests if "__pycache__" not in f]
    tests = sorted([f for f in tests if (PATH_TO_REPO / f).is_dir() or f.startswith("tests/test_")])
    tests = [f for f in tests if (PATH_TO_REPO / f).exists()]

    return teststhe License.
